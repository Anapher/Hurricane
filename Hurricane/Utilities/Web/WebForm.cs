...
namespace Hurricane.Utilities.Web
{
    internal sealed class WebForm
    {
        ...
        {
            string uri = string.Join("&", _inputs.Select(x => string.Format("{0}={1}", x.Key, x.Value)).Select(i => i.ToString()).Where(s => !string.IsNullOrEmpty(s)).ToArray());
